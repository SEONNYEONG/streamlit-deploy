...
# ======= STEP 01. 결측치, 이상치 확인 및 처리 - 데이터셋 불러와서 우선 실시 ======= #
...
# ======= STEP 02. 데이터 타입 변환 및 정리 ======= #
## Label Encoding(레이블 인코딩)은 문자열(범주형 데이터)을 숫자로 변환하는 방법입니다
le_label = LabelEncoder() # 레이블(= 타겟(결과치 or 예측치))이 과체중이냐 비만이냐 저체중이냐 등을 숫자화 하는 것이다
df['Label'] = le_label.fit_transform(df['Label']) # 인코딩을 해서 입력 받은 것을 다시 숫자화 하는 것이다
...
y = df['Label'] # 숫자화하여 랩핑 된 상태

# ======= STEP 04. 데이터 정규화(Scaling, 표준화) ======= #
...
# ======= STEP 05. 데이터셋 분할(train - test Split) ======= #
## 머신러닝 모델이 학습(Training) 과 평가(Testing) 를 위해 데이터를 분리해야 함.
## train_test_split() 함수를 사용하여 80%는 학습 데이터, 20%는 테스트 데이터로 분리합니다. (보통 80% 와 20%로 나눔!)
X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size = 0.2, random_state = 42) # random_state = 42 랜덤으로 80% 20% 데이터를 뽑지만 돌릴때마다 그 데이터를 사용
...
# ======= STEP 07. 예측 및 성능평가(Model Prediction & Evaluation) ======= #
def classification_report_to_df(report):
    df_report = pd.DataFrame(report).transpose();
    return df_report

y_pred = model.predict(X_test)
accuracy = accuracy_score(y_test, y_pred) # 두개가 만점이면 1이 나온다
repot_dict = classification_report(y_test, y_pred, target_names = le_label.classes_, output_dict = True)
classification_df = classification_report_to_df(repot_dict)
...
## F1-score : precision와 recall의 조화평균 => 모델의 전체적인 성능을 나타냄
## support 각 클래스(Label)에 속하는 샘플의 갯수 => 데이터셋 내에 존재하는 해당 클래스갯수
...
def home():
    ...
    st.markdown("""
        ...
        - **Weight** : 몸무게(kg)
        - **BMI** : 체질량지수
        - **Label** : 비만여부(Normal Weight, Overweight, Obese, Underweight)
    """)
# ======= EDA(데이터시각화화면) ======= #
def eda():
    st.title('데이터 시각화')
    ...
    with chart_tabs[0]:
        ...
        for i, col in enumerate(columns):
            ax = axes[i//2, i%2] 
            sns.histplot(df[col], bins = 20, kde = True, ax = ax) # bins = 20란? 구간(20개)씩 끊는다는 의미
            ax.set_title(col)
        st.pyplot(fig)
